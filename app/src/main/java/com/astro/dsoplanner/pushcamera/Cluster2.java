...
public class Cluster2 {
    ...
    public Cluster2(short[][] data2, double sum, boolean mirror, int w, int h, boolean r_bright_areas) {
        ...
        this.w = w;
        this.h = h;
        this.r_bright_areas = r_bright_areas;
    }
    ...
    private Map<Integer, List<P2i>> get_bright_areas(short[][] data, int threshold, int eps, int m, int decimation, int min_pixels_in_area) {
        ...
        for (int i = 0; i < rows; i++) {
            ...
            if (Thread.currentThread().isInterrupted()) {
                print("get_bright_areas 1, interrupted");
                return null;
            }
        }
        ...
        if (clusters == null) {
            print("get_bright_areas 2, interrupted");
            return null;
        }
        ...
        for (Map.Entry<Integer, List<P2i>> e : clusters.entrySet()) {
            ...
            for (P2i p : v) {
                ...
                for (int k = 0; k < 2 * decimation; k++) {
                    ...
                    if (Thread.currentThread().isInterrupted()) {
                        print("remove_bright_areas, interrupted");
                        return null;
                    }
                }
     ...
     * @return null if interrupted
     */
    public Holder2<P2i, Map<Integer, P>> run() {
        ...
        if (crop == null) {
            for (int i = 0; i < rows; i++) {
                ...
                if (Thread.currentThread().isInterrupted()) {
                    print("cluster run, interrupted 1");
                    return null;
                }
            }
        } else {
            for (int i = row_low; i < row_high; i++) {
                ...
                if (Thread.currentThread().isInterrupted()) {
                    print("cluster run, interrupted 2");
                    return null;
                }
            }
        }
        ...
        while (true) {
            ...
            if (Thread.currentThread().isInterrupted()) {
                print("cluster run, interrupted 3");
                return null;
            }
            clusters.remove(DbScan.NOISE);
            for (Map.Entry<Integer, List<P2i>> e : clusters.entrySet()) {
        ...
        return new Holder2<>(new P2i(rows, cols), data);
